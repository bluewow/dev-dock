...
팀을 소환하기 전에 리더가 사용자와 짧은 Q&A를 통해 방향을 정렬한다.
...
> 할당: 리더1, 기획자2, 디자이너2, QA1

1. **리더가 스캔 후 context.md 생성** (팀원 소환 전 필수, 기존 context.md가 있으면 생략):
   - 기존 화면 목록, 레이아웃 패턴
   - 컬러 팔레트, 타이포그래피, 간격 체계
...
2. 기획자A, 기획자B를 소환하여 병렬로 기획안 작성 요청
3. 두 기획안을 수신 후 리더가 통합/조율
4. 디자이너A, 디자이너B 병렬로 디자인 시안 작성 (context.md에서 디자인 시스템 정보 참조)
5. QA에게 기획 플로우 검증 요청
6. 리더가 내부 검토 후 태스크 폴더에 산출물 생성:
...
3. 디자이너 소환 (context.md 참조 + Before/After 산출물 규칙 전달)
